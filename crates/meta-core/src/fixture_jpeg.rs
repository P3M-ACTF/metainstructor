...
impl TiffBuf {
    ...
    fn write_ifd(&mut self, entries: &[IfdEntry]) -> usize {
        ...
        self.data.resize(self.data.len() + entries.len() * 12 + 4, 0);
        // next IFD = 0 already
...
fn build_rich_tiff() -> (Vec<u8>, bool) {
    ...
    buf.data.extend_from_slice(&ifd0_entries_planned.to_le_bytes());
...
const MINIMAL_JPEG_BODY: &[u8] = &[
    ...
    0x08, 0x00, 0x08, 0x01, 0x01, 0x11, 0x00, 0xFF, 0xC4, 0x00, 0x1F, 0x00, 0x00, 0x01, 0x05,
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
    0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0xFF, 0xC4, 0x00, 0x1F, 0x10,
    ...
    0x61, 0xFF, 0xDA, 0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x3F, 0x00, 0x7F, 0xFF, 0xD9,
];
